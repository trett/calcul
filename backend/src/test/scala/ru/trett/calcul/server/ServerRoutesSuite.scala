...
class ServerRoutesSuite extends FunSuite:
  ...
  test("Settings routes reject unauthenticated requests with 401 Unauthorized") {
    ...
    try
      ...
      val saveResult = routes.saveGeminiKeyRoute.logic(sttp.monad.IdentityMonad)(())(
        (None, ru.trett.calcul.model.SaveGeminiKeyRequest("test-key"))
      )
  ...
  test("Settings routes allow authenticated user to manage key") {
    val conn = TestPostgresContainer.newConnection()
    try
      val mockGemini = new ru.trett.calcul.ai.GeminiService:
        override def validateKey(key: String): Either[String, Unit] =
          if key == "valid-secret-key-1234" then Right(())
          else Left("Bad key")
      val routes = new ServerRoutes(
        transactor = ru.trett.calcul.db.DbTransactor.fromConnection(conn),
        gemini = mockGemini
      )
      val user = ru.trett.calcul.model.User(
      ...
      assertEquals(getRes1, Right(ru.trett.calcul.model.GeminiKeyStatus(hasKey = false, maskedKey = None)))
      ...
      val emptyRes = routes.saveGeminiKeyRoute.logic(sttp.monad.IdentityMonad)(())(
        (Some(token), ru.trett.calcul.model.SaveGeminiKeyRequest("invalid"))
      )
      assert(emptyRes.isLeft)

      // Valid key succeeds
      val saveRes = routes.saveGeminiKeyRoute.logic(sttp.monad.IdentityMonad)(())(
        (Some(token), ru.trett.calcul.model.SaveGeminiKeyRequest("valid-secret-key-1234"))
      )
      ...
      assertEquals(
        getRes2,
        Right(ru.trett.calcul.model.GeminiKeyStatus(hasKey = true, maskedKey = Some("••••••••••••1234")))
      )
      ...
      assertEquals(getRes3, Right(ru.trett.calcul.model.GeminiKeyStatus(hasKey = false, maskedKey = None)))
    finally conn.close()
  }
  ...
  test("Meal, calorie, and weight routes reject unauthenticated requests with 401 Unauthorized") {
    ...
    try
      ...
      val mealReq = ru.trett.calcul.model.CreateMealRequest(
        mealDate = date,
      ...
      val targetReq = ru.trett.calcul.model.SetTargetRequest(date, 2000)
      val weightReq = ru.trett.calcul.model.RecordWeightRequest(date, BigDecimal("75.0"), "kg")
  ...
  test(
    ...
    try
      ...
      val targetRes = routes.setDailyTargetRoute.logic(sttp.monad.IdentityMonad)(())(
        (Some(token), ru.trett.calcul.model.SetTargetRequest(date, 2400))
      )
      ...
      val mealReq = ru.trett.calcul.model.CreateMealRequest(
        ...
        items = List(
          ru.trett.calcul.model.CreateMealItem("Avocado Toast", 445),
          ru.trett.calcul.model.CreateMealItem("Black Coffee", 5)
        )
      )
      ...
      // 5. Record weight
      val weightReq = ru.trett.calcul.model.RecordWeightRequest(date, BigDecimal("76.80"), "kg")
